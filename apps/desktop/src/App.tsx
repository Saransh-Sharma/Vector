  ...
  return <div className="cockpit">
    ...
    <div className="cockpit-grid">
      <section className="onboarding-panel" aria-labelledby="setup-title">
        ...
        {onboardingStep === 1 && <SetupStep title="Find LM Studio" text="Read the exact model IDs currently exposed on 127.0.0.1. Nothing leaves this machine." action={lm.isFetching ? "Scanning LM Studio…" : "Discover models"} onAction={() => setDiscover(true)} disabled={lm.isFetching} detail={lm.data ? `${lm.data.models.length} model${lm.data.models.length === 1 ? "" : "s"} found · ${lm.data.latencyMs} ms` : lm.isError ? "LM Studio is not reachable. Start its local server and retry." : "No API keys required."} />}
